<template>
  <ul v-if="posts.length > 0" class="cards">
    <li class="home-list" v-for="(post, index) in posts" :key="index">
      <nuxt-link :to="`/${postType}/${post.slug}`" class="card card--clickable">
        <template v-if="postType === 'projects'">
          <span class="flex-1">
            <h6
              class="inline-block py-1 px-2 mr-1 text-dark dark:text-primary-400 text-sm font-medium rounded-sm date"
            >
              {{ post.category }}
            </h6>
            <h3 class="card-title">{{ post.title }}</h3>
            <p class="mt-2 card-description">{{ post.description }}</p>
          </span>
        ...
        <template v-else>
          <span class="w-full">
            <span class="flex justify-between align-baseline">
              ...
              <h6
                v-if="post.createdAt"
                class="self-start inline-block mt-0 py-1 px-2 text-xs text-dark dark:text-white font-medium rounded-sm whitespace-no-wrap date"
              >
